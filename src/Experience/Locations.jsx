...
export const Locations = () => {
  const { yPositionCamera, setIsModalOpen, setGroupData } = useMap();

  return userGroups.map((userGroup, i) => {
    return (
      <Html
        key={userGroup.id}
        position={userGroup.coordinates}
        className="relative"
        style={{ zIndex: i }}
      >
        <div
          ...
          style={{
            width: `${Math.max(20, 100 * (1 - yPositionCamera / 2))}px`,
            height: `${Math.max(20, 100 * (1 - yPositionCamera / 2))}px`,
            transform: `scale(${1 - yPositionCamera * 0.3})`,
          }}
          onClick={() => {
            console.log("userGroup", userGroup);

            setIsModalOpen(true);
            setGroupData(userGroup.id - 1);
          }}
        >
          <div
            className="container-webgl absolute top-0 left-0 z-0 flex h-full w-full items-center justify-center rounded-full border border-white transition-all"
            style={{
              scale: yPositionCamera > 0.5 ? 1 : 0,
            }}
          >
            <svg
          ...
          <img
            src={userGroup.logo}
            alt="user gruup logo"
            className="user-select-none absolute top-1/2 left-1/2 z-10 h-full w-full -translate-x-1/2 -translate-y-1/2 rounded-full object-cover transition-all"
            style={{
              scale: yPositionCamera < 0.5 ? 1 : 0,
            }}
          />
